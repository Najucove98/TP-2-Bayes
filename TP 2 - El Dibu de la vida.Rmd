...
Para generar una muestra de valores $\{y^{(1)}, y^{(2)},..., y^{(n)}\}$ a partir de una distribución de probrabilidad $P$ con función de densidad $p$.

$1.\text{Seleccionar un punto inicial } y^{(1)}.$

$2.\text{Para cada } t \in \{1,..., n\} \text{, repetir:}$

$\text{i.Realizar propuesta}$

SEGUIR CON ESTA IDEA?
...
...
...
...
...
$\text{Transformación de la variable aleatoria}$

$\text{Sea }Y = g(X) = log(x)$
$\text{La función }g:(0,\ +\infty) \rightarrow \mathbb{R}$

$\text{Hallamos la inversa: }$
$\text{Se tiene }g^{-1}(X) = e^{X}$
$\text{Donde }g^{-1}: \mathbb{R} \rightarrow (0,\ +\infty)$

$\text{Luego se tiene que: } P_Y(y) = P_X(g^{-1}(y)) \left| \frac{d}{dy} g^{-1}(y) \right|$
$\text{Luego se tiene que: } P_Y(y) = P_X(e^{y}) \left| \frac{d}{dy} e^{y} \right|$
...
muestra <- metropolis_hastings_normal(n = 10000,
                           x = 0,
                           sigma = 0.2,
                           p = py)
