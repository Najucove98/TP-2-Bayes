...
```{r f6,echo=FALSE, warning=FALSE, message=FALSE, fig.align = "center",  fig.width = 4, fig.height = 2.8}
set.seed(0398)
...
```{r f7,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Traceplots con σ = 1.3"}
set.seed(0398)
# Creamos una lista para guardar 4 cadenas de muestras con el metodo usado y una propuesta normal con sigma 0.2
muestras_md <- list()
n_muestras <- 10000
...
```{r f8,echo=FALSE, warning=FALSE, message=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8}
set.seed(0398)
# se juntan las cadenas en una matriz para obtener las medidas de diagnostico
muestras_matriz <- matrix(df_muestras_md$valor, ncol = n_cadenas, byrow = FALSE)
...
```{r f9,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Histograma de la muestras de Y"}
set.seed(2198)
...
```{r f11,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Histograma de la muestras de X"}
...
```{r f12,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Traceplot de la muestras de X"}
...
```{r f13,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Histograma de la muestras de Y"}
graficar_distribucion(muestralog, grid_y, grid_p_y, xlab = "y", ylab = TeX("$f_Y(y)$"), color = "#44A699", color_l = "#44A699")
```


```{r f14,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Traceplot de la muestras de Y"}
...
```{r f16,echo=FALSE, warning=FALSE, message=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8}
# se juntan las cadenas en una matriz para obtener las medidas de diagnostico
...
```{r f17,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Histograma de la muestras de X"}
...
```{r f18,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Traceplot de la muestras de Y"}
graficar_traza(muestras_x, color = "#3b78b0")                           
```
...
$$p(\mu, \sigma^* \mid y) \propto\left[ \sum_{i = 1}^{N} P_y(y_i \mid \mu,\exp(\sigma^*))\right]*P_\mu(\mu)*P_\sigma^*(exp(\sigma^*))*\exp(\sigma^*)$$
...
sigma <- diag(2)* (0.05**2) # Propuesto en clase

# Obtener muestras
muestras_theta_estrella <- metropolis_hastings_log(
  logp = logp,
  x = c(0.5, log(1.5)), # Dos puntos iniciales
  n = 10000,
  sigma = sigma
)
...
#traceplot de mu
plot_trace(muestras_theta_estrella[ ,1], color = "green", ylab = "mu")
```

```{r f15a,echo=FALSE, fig.align = "center", fig.width = 5, fig.height = 3.5, warning=FALSE, fig.cap="Traceplots con sigma = 1.3"}
set.seed(0398)
...
df_muestras_mu <- df_muestras_md_log[c(1:10000,20001:30000,40001:50000,60001:70000), ]


#dataframe con las muestras de sigma 
df_muestras_sigma <- df_muestras_md_log[c(10001:20000,30001:40000,50001:60000,70001:80000), ]

```


```{r ,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Traceplots con sigma = 1.3"}
# Grafica las 4 trazas
...
Nueva propuesta para mu

```{r , echo=FALSE, warning=FALSE, message=FALSE}
set.seed(0398)
# Creamos una lista para guardar 4 cadenas del vector de parametros
muestras_md_log2 <- list()
n_muestras <- 10000
n_cadenas <- 4
x_inicio <- 0
sigma <- diag(2)* (0.12**2)
...
# Se llama a la función `metropolis_hastings_normal` y se guardan los resultados en una lista.
for (j in seq_len(n_cadenas)) {
  muestras_md_log2[[j]] <- metropolis_hastings_log(
    n = n_muestras,
    x = c(1, log(1.5)),  # Dos puntos iniciales
    logp = logp,
    sigma = sigma # matriz de 2x2 diagonal
  )
}
...
# creamos un data frame para poder graficar
df_muestras_md_log2 <- crear_df_muestras(muestras_md_log2)

#dataframe con las muestras de mu
df_muestras_mu2 <- df_muestras_md_log2[c(1:10000,20001:30000,40001:50000,60001:70000), ]

```

```{r ,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Traceplots con sigma = 1.3"}
# Grafica las 4 trazas
graficar_trazas_md(df_muestras_mu2, ylab = "mu", colors = c("#87CEEB", "#FFA07A", "#FFFFE0", "#B0E0E6"))
```


```{r ,echo=FALSE, warning=FALSE, message=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8}
# se juntan las cadenas en una matriz para obtener las medidas de diagnostico
muestras_matriz <- matrix(df_muestras_mu2$valor, ncol = n_cadenas, byrow = FALSE)
...
Mejores resultados con un desvio de 0.12 y mismo valor inicial
...
plot_trace(exp(muestras_theta_estrella[ ,2]), color = "orange", ylab = "sigma")
```
...
graficar_trazas_md(df_muestras_sigma_trp, ylab = "sigma", colors = c("#87CEEB", "#FFA07A", "#FFFFE0", "#B0E0E6"))
```
...
set.seed(0398)
# Creamos una lista para guardar 4 cadenas del vector de parametros
muestras_md_log3 <- list()
n_muestras <- 10000
n_cadenas <- 4
x_inicio <- 0
sigma <- diag(2)* (0.13**2) #anterior 0.12 <- 5300 muestras efectivas 1.5 <- 5350

# Se llama a la función `metropolis_hastings_normal` y se guardan los resultados en una lista.
for (j in seq_len(n_cadenas)) {
  muestras_md_log3[[j]] <- metropolis_hastings_log(
    n = n_muestras,
    x = c(0.5, log(0.8)),  # Dos puntos iniciales
    logp = logp,
    sigma = sigma # matriz de 2x2 diagonal
  )
}
...
# creamos un data frame para poder graficar
df_muestras_md_log2 <- crear_df_muestras(muestras_md_log3)

#dataframe con las muestras de sigma
df_muestras_sigma2 <- df_muestras_md_log2[c(10001:20000,30001:40000,50001:60000,70001:80000), ]

df_muestras_sigma_trp2 <- df_muestras_sigma2
df_muestras_sigma_trp2[ ,3] <- exp(df_muestras_sigma2[ ,3])

```

```{r ,echo=FALSE, fig.align = "center", fig.width = 4, fig.height = 2.8, warning=FALSE, fig.cap="Traceplots con sigma = 1.3"}
# Grafica las 4 trazas
graficar_trazas_md(df_muestras_sigma_trp2, ylab = "mu", colors = c("#87CEEB", "#FFA07A", "#FFFFE0", "#B0E0E6"))
```
...
set.seed(0398)
# se juntan las cadenas en una matriz para obtener las medidas de diagnostico
muestras_matriz <- matrix(df_muestras_sigma_trp2$valor, ncol = n_cadenas, byrow = FALSE)
...
la nueva propuesta de un desvio de 0.13 y valor inicial de log(0.75) parece ser una mejor opcion para obtener muestras de sigma
...
```{r ,echo=FALSE, warning=FALSE, message=FALSE, fig.align = "center", fig.width = 6, fig.height = 7, fig.cap="Histograma de la muestras de X"}
titulo_fila1 <- textGrob("Histograma y Boxplot para Control", gp = gpar(fontsize = 14, fontface = "bold"))
titulo_fila2 <- textGrob("Histograma y Boxplot para Ozone", gp = gpar(fontsize = 14, fontface = "bold"))

# Armar el layout con títulos por fila
grid.arrange(
  titulo_fila1,           # título para fila 1
  arrangeGrob(plot_con_hist, plot_con_box, ncol = 2),  # gráficos fila 1
  titulo_fila2,           # título para fila 2
  arrangeGrob(plot_oz_hist, plot_oz_box, ncol = 2),  # gráficos fila 2
  ncol = 1, heights = c(0.4, 1, 0.4, 1)  # ajusta altura de títulos
)

```

```{r,echo=FALSE, warning=FALSE, message=FALSE, fig.align = "center", fig.width = 7, fig.height = 3.5, fig.cap="Histograma y Boxplot para Control"}
...
## Conclusion mia
En base a las medidas descriptivas se puede concluir que el cambio en el peso de las ratas control y el cambio de peso para las ratas ozono tienen diferencias muy marcadas ya que el cambio de peso medio para las ratas de ozone son aproximadamente la mitad que el cambio de peso medio para las ratas control, es decir el peso medio de las ratas de ozone se redujo aproximadamente a la mitad. Ademas con los boxplot del cambio de peso para las ratas de ozone se puede concluir que son aproximadamente asimetricos por izquierda, mientras que el cambio de peso de las ratas control es muy concentrado


## Modificada por chatgpt
En base a las medidas descriptivas obtenidas, se observa una diferencia considerable entre el cambio de peso en las ratas del grupo control y en las ratas expuestas a ozono.El cambio de peso medio en las ratas expuestas a ozono representa aproximadamente la mitad del registrado en las ratas del grupo control, lo que indica una reducción en la ganancia de peso atribuible a la exposición al ozono.

Asimismo, el análisis mediante diagramas de caja (boxplots) revela que la distribución del cambio de peso en las ratas del grupo ozono presenta una asimetría hacia la izquierda, lo cual sugiere la presencia de valores más bajos en comparación con la mediana. 
En contraste, el cambio de peso en las ratas del grupo control muestra una distribución más concentrada, lo que indica una mayor homogeneidad en los datos de ese grupo.
...
Dadas una misma media y varianza, tanto la distribución normal como la t de Student con 3 grados de libertad son simétricas respecto a su media. Sin embargo, presentan diferencias importantes:

1) Colas y valores extremos

* La distribución normal decreció rápidamente a medida que se alejaba del centro, lo que implicó que los valores extremos tuvieran una probabilidad muy baja.

* En cambio, la t de Student con 3 grados de libertad presentó colas más pesadas, es decir, asignó una mayor probabilidad a valores extremos. Esto significó que fue más probable observar datos alejados del centro bajo esta distribución.

2) Dispersión de los datos

* Los valores generados por una normal estuvieron más concentrados alrededor de la media, lo que se reflejó en una menor dispersión.

* En cambio, la t de Student mostró mayor variabilidad, especialmente en los extremos. Aunque la varianza fue la misma numéricamente, la mayor dispersión efectiva afectó a las estadísticas inferenciales como los intervalos de confianza, haciéndolos más amplios.

3) Uso en contextos con poca información

* Cuando se trabajó con muestras pequeñas o con incertidumbre sobre la varianza, la t de Student fue preferible.

* Su forma con colas pesadas permitió capturar mejor la variabilidad e incertidumbre en situaciones donde los datos fueron escasos o ruidosos, protegiendo contra la subestimación del error.
...
```{r}
# DEBE SER UNA FUNCION PARECIDA AL ITEM 5 QUE TENGA DE ARGUMENTOS UN VECTOR DE PARAMETROS PERO DE DIMENSION 5

# Función de densidad posterior en escala logarítmica (sin normalizar)
log_posterior <- function(params, y_vector, normal = TRUE) {
  # 'params' tiene longitud 2
  mu <- params[1,2]
  sigma_estrella <- params[3,4]
  sigma <- exp(sigma_estrella) # Salida sigma debe ser positiva
  ...
  if(normal == T){
    ...
    # Log-verosimilitud (asumiendo datos independientes)
    log_likelihood <- sum(dt(y_vector, mean = mu, sd = sigma, log = TRUE))

  }

  # Log-prior de mu
  log_prior_mu1 <- dnorm(mu[1], mean = 0, sd = 1, log = TRUE)

  # Log-prior de mu
  log_prior_mu2 <- dnorm(mu[2], mean = 0, sd = 1, log = TRUE)
  
  # Log-prior de sigma (transformado) - Incluyendo el Jacobiano
  log_prior_sigma1 <- dgamma(sigma[3], shape = 2, rate = 2, log = TRUE) + sigma_estrella[1]

  # Log-prior de sigma (transformado) - Incluyendo el Jacobiano
  log_prior_sigma2 <- dgamma(sigma[4], shape = 2, rate = 2, log = TRUE) + sigma_estrella[2]
  
  # Log-posterior no normalizado
  log_posterior_value <- log_likelihood + log_prior_mu1 + log_prior_mu2 + log_prior_sigma1 + log_prior_sigma2
