...
$$p(\mu_o,\mu_c, \sigma_o^, \sigma_c^\ | \textbf{y}) \propto p( \textbf{y}  \text{ | } \mu_o,\mu_c, \sigma_o, \sigma_c).p_{\mu_o}(\mu_o).p_{\mu_c}(\mu_c).p_{\sigma_o^}(\sigma_o^) p_{\sigma_c^}(\sigma_c^)$$
...
$$p(\mu_o,\mu_c, \sigma_o^, \sigma_c^\ | \textbf{y}) \propto\left[ \sum_{i = 1}^{N} p_y(y_i \mid \mu_o, \mu_c,\exp(\sigma_o^)*exp(\sigma_c^))\right] + \log{p_{\mu_o}(\mu_o)} + p_{\sigma_o}^(exp(\sigma_o^)) + \sigma_o^* + \log{p_{\mu_c}(\mu_c)} + p_{\sigma_c}^(exp(\sigma_c^)) + \sigma_c^*$$
