...
```{r f15,echo=FALSE, fig.align = "center", fig.width = 5, fig.height = 3.5, warning=FALSE, fig.cap="Traceplots con sigma = 1.3"}
set.seed(0398)
# Creamos una lista para guardar 4 cadenas de muestras con el metodo usado y una propuesta normal con sigma 0.2
...
# dim(muestras_theta_estrella) son 10000 muestras para cada parametro del vector theta
```
...
for (j in seq_len(n_cadenas)) {
  muestras_md_log3[[j]] <- metropolis_hastings_log(
    n = n_muestras,
    x = c(0.5, log(0.75)),  # Dos puntos iniciales
    logp = logp,
    sigma = sigma # matriz de 2x2 diagonal
  )
}
...
# datos_control %>%
#   summarise(
#     Media_mpg   = mean(datos_control$gain),
#     Mediana_mpg = median(datos_control$gain),
#     DesvStd_mpg = sd(datos_control$gain),
#     Min_mpg     = min(datos_control$gain),
#     Max_mpg     = max(datos_control$gain)
#   ) %>%
#   kable(caption = "Tabla de medidas descriptivas de datos_control")
...
# datos_oz %>%
#   summarise(
#     Media_mpg   = mean(datos_oz$gain),
#     Mediana_mpg = median(datos_oz$gain),
#     DesvStd_mpg = sd(datos_oz$gain),
#     Min_mpg     = min(datos_oz$gain),
#     Max_mpg     = max(datos_oz$gain)
#   ) %>%
#   kable(caption = "Tabla de medidas descriptivas de datos de ozone")
