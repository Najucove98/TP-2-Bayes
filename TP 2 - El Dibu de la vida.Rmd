...
log_posterior_oz <- function(params, y_vector, normal = TRUE) {
  ...
  if(normal == T){
    ...
    # Log-verosimilitud (asumiendo datos independientes)
    log_likelihood_oz <- sum(dt(y_vector, mean = mu_oz, sd = sigma_oz, log = TRUE))
...
log_posterior_cont <- function(params, y_vector, normal = T) {
  ...
  if(normal == T){
    ...
    # Log-verosimilitud (asumiendo datos independientes)
    log_likelihood_cont <- sum(dt(y_vector, mean = mu_cont, sd = sigma_cont, log = TRUE))
...
logp_oz <- partial(log_posterior_oz, y_vector = y_datos)

logp_cont <- partial(log_posterior_cont, y_vector = y_datos)



log_posterior_general <- function(params){
  params1 <- params[1:2]
  params2 <- params[3:4]
  return(log_posterior_oz(params1) + log_posterior_cont(params2))
}
```
